...
const Ejercicio2 = () => {
  const formula1 = "$$1+3+5+7+9+11+13+15+...$$";
  const formula2 = "$$\\sum_{i=1}^n 2i-1 =n^2$$";
  ...
  const handleSubmit = (event) => {
    ...
    for (let i = 1; i <= input; i++) {
      i === 1
        ? (str = str.concat(`$$${2 * i - 1}$$`))
        : (str = str.concat(`$$+${2 * i - 1}$$`));
      resultado1 += 2 * i - 1;
    }
    str = str.concat(`$$=${resultado1}$$`);
    setFormula3(str);
    let resultado2 = input * input;
    setFormula4(
      `$$\\sum_{i=1}^{${input}} 2i-1 =${input}^2 =${resultado2}$$`
    );
  };
